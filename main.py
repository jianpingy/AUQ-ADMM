...
DATASET_NAME = 'MNIST'
LOSS_NAME = 'Elastic_Net' #'Elastic_Net' or 'Multinomial' or 'Smoothed_SVM', 
...
def g(z):
    return rho1*torch.norm(z,p=1)+rho2*0.5*((z*z).sum()) #this g is particularly for elastic net regression (rho1=1, rho2=1)

##AUQADMM SETUP
auqadmm = AUQADMM(params, g , rho1, rho2, trainsets, LOSS_NAME)
a = 0.1; b = 1.0 #Initial Restriction interval

##OPTIMIZATION
#AUQADMM
result_auqadmm = auqadmm.fit(maxiter=10, X_EPOCHS=30, abs_tol=1e-4, rel_tol=1e-5, rank=10, a=a, b=b, K=1)

#Other ADMM Algs: CADMM or RBADMM or ACADMM
# admm = ADMM(params, g , rho1, rho2, trainsets, LOSS_NAME)
# print('%s   %s    %s          %s          %s'
#           % ('Iter',    'f(z) + g(z)',    'p_res',     'd_res',     'time used'))
# result_admm = admm.do(maxiter=10, X_EPOCHS=30, epsilon=0.2, start_tau=1.0, T_f=2, C_cg=1e10, method='CADMM')
